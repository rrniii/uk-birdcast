...
def merge(
    manifest: Path, reports_root: Path, output: Path, *, release_id: str
) -> dict[str, object]:
    chunks = [
        json.loads(line)
    ...
    failed = sorted(
        index for index, payload in reports.items() if payload.get("status") != "passed"
    )
    stale = sorted(
        index for index, payload in reports.items() if payload.get("release_id") != release_id
    )
    mismatched = sorted(
        index
    ...
    payload = {
        ...
        "source_day_count": sum(
            int(report.get("source_day_count") or 0) for report in reports.values()
        ),
        "hourly_row_count": sum(
            int(report.get("hourly_row_count") or 0) for report in reports.values()
        ),
