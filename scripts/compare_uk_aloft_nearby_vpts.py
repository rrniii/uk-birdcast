...
def nearest_profile(
    profiles: dict[datetime, dict[float, dict[str, str]]], target: datetime
) -> tuple[datetime, dict[float, dict[str, str]]]:
...
def statistics(
    pairs: list[tuple[float, float]], *, log_ratio: bool = False
    ...
    return {
        ...
        "median_log10_uk_to_aloft_ratio": median([math.log10(value) for value in ratio])
        if log_ratio and ratio
        else None,
    }


def compare_pair(
    pair_id: str, pair: dict[str, object], aloft_dir: Path, uk_cache_dir: Path
) -> dict[str, object]:
        ...
        for aloft_time, aloft_rows in aloft_profiles.items():
            ...
            for height in common_heights:
                for variable in VARIABLES:
                    uk_value, aloft_value = (
                        finite(uk_rows[height].get(variable)),
                        finite(aloft_rows[height].get(variable)),
                    )
    ...
    return {
        ...
        **pair,
        ...
        "variables": {
            name: statistics(values, log_ratio=name == "dens") for name, values in raw.items()
        },
    }
...
def main() -> None:
    ...
    results = [
        compare_pair(pair_id, PAIRS[pair_id], args.aloft_vpts_dir, args.uk_cache_dir)
