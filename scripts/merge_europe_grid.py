...
def merge(grid_root: Path, output: Path, *, start_day: str, end_day: str) -> dict[str, object]:
    expected = [date.fromisoformat(start_day) + timedelta(days=index) for index in range((date.fromisoformat(end_day) - date.fromisoformat(start_day)).days + 1)]
    ...
    try:
        ...
        con.execute(f"COPY (SELECT * FROM read_parquet({source}, union_by_name=true)) TO ? (HEADER, DELIMITER ',')", [str(output)])
        rows = con.execute(f"SELECT count(*) FROM read_parquet({source}, union_by_name=true)").fetchone()[0]
    finally:
        con.close()
    return {"status": "passed", "day_count": len(files), "row_count": int(rows), "output": str(output)}
...
if __name__ == "__main__":
    ...
    print(merge(Path(args.grid_root), Path(args.output), start_day=args.start_day, end_day=args.end_day))
