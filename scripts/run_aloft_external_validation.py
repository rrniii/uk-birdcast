...
def evaluate(vpts_dir: Path, prediction_csv: Path, pulse: str, site_ids: set[str]) -> dict:
    ...
    per_site = {
        site: {"observations": [], "predictions": [], "files": 0, "valid_hours": 0}
...
def main() -> None:
    ...
    output = {
        ...
        "evaluations": {
            pulse: evaluate(args.vpts_dir, prediction_paths[pulse], pulse, site_ids)
            for pulse in pulses
        },
    }
    args.output.write_text(json.dumps(output, indent=2) + "\n")
    print(
        json.dumps({"output": str(args.output), "available_vpts_file_count": available}, indent=2)
    )
