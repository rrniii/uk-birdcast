...
def test_era5_fidelity_merge_requires_every_day(tmp_path: Path) -> None:
    ...
    reports.mkdir()
    (reports / "20250714.json").write_text(
        '{"status":"passed","release_id":"release-a","radars_sha256":"abc"}', encoding="utf-8"
    )
    ...
    (reports / "20250715.json").write_text(
        '{"status":"passed","release_id":"other","radars_sha256":"abc"}', encoding="utf-8"
    )
