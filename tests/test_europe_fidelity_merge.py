...
def test_source_fidelity_merge_requires_every_locked_chunk(tmp_path: Path) -> None:
    ...
    (reports / "0.json").write_text(
        json.dumps(
            {"status": "passed", "source_day_count": 2, "hourly_row_count": 3, "chunk": first}
        ),
        encoding="utf-8",
    )
    merge = _load().merge
    ...
    (reports / "1.json").write_text(
        json.dumps({"status": "passed", "release_id": "other-release", "chunk": second}),
        encoding="utf-8",
    )
    try:
        merge(manifest, reports, tmp_path / "summary.json", release_id="release-a")
    ...
    (reports / "1.json").write_text(
        json.dumps({"status": "passed", "release_id": "release-a", "chunk": first}),
        encoding="utf-8",
    )
    try:
        merge(manifest, reports, tmp_path / "summary.json", release_id="release-a")
