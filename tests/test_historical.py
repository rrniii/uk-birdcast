...
def test_build_historical_products_keeps_pulses_separate_and_builds_high_res_map(
    tmp_path: Path,
) -> None:
        ...
        [
            {
                ...
                "pulse": "lp",
                "profile_count": 10,
                "vid_birds_per_km2": 12,
                "mean_vid_birds_per_km2_per_profile": 1.2,
                "mean_weighted_height_m": 800,
                "mean_ff_ms": 10,
            },
            {
                "radar": "test-radar",
    ...
    annual_fields = [
        "day_count",
        "mean_daily_vid_birds_per_km2",
        "mean_profile_vid_birds_per_km2",
        "profile_count",
        "pulse",
        "radar_count",
        "season",
        ...
        for season in ("spring", "autumn"):
            for period in ("day", "civil_twilight", "night"):
                annual_rows.append(
                    {
                        "day_count": 10,
                        "mean_daily_vid_birds_per_km2": 10 * multiplier,
                        "mean_profile_vid_birds_per_km2": multiplier,
                        "profile_count": 100,
                        "pulse": pulse,
                        "radar_count": 1,
                        "season": season,
                        "solar_period": period,
                        "vid_birds_per_km2": 100 * multiplier,
                        "year": 2025,
                    }
                )
    ...
    _write_csv(
        ...
        [
            {
                "day_count": 30,
                "duration_days_10_90": 20,
                "early_10_date": "2025-03-01",
                "late_90_date": "2025-04-01",
                "median_50_date": "2025-03-15",
                "onset_5_date": "2025-02-25",
                "peak_date": "2025-03-16",
                "peak_vid_birds_per_km2": 5,
                "pulse": "lp",
                "radar": "test-radar",
                "season": "spring",
                "solar_period": "night",
                "vid_birds_per_km2": 100,
                "year": 2025,
            },
            {
                "day_count": 30,
                "duration_days_10_90": 20,
                "early_10_date": "2025-09-01",
                "late_90_date": "2025-11-01",
                "median_50_date": "2025-10-01",
                "onset_5_date": "2025-08-25",
                "peak_date": "2025-10-02",
                "peak_vid_birds_per_km2": 8,
                "pulse": "lp",
                "radar": "test-radar",
                "season": "autumn",
                "solar_period": "night",
                "vid_birds_per_km2": 150,
                "year": 2025,
            },
        ],
    )
                ...
                "features": [
                    {
                        ...
                        "geometry": {
                            "type": "MultiPolygon",
                            "coordinates": [[[[-2, 50], [-1, 50], [-1, 51], [-2, 50]]]],
                        },
                    },
                    {
                        "type": "Feature",
                        "properties": {"ADM0_A3": "IRL"},
                        "geometry": {
                            "type": "MultiPolygon",
                            "coordinates": [[[[-8, 52], [-7, 52], [-7, 53], [-8, 52]]]],
                        },
                    },
                ],
            }
    ...
    latest_path = output / "latest/historical.json"
    manifest = json.loads(latest_path.read_text(encoding="utf-8"))
    daily = json.loads(
        (output / manifest["assets"]["daily_by_year"]["2025"]).read_text(encoding="utf-8")
    )
    map_data = json.loads((output / manifest["assets"]["boundary"]).read_text(encoding="utf-8"))
    annual_plot = (output / manifest["assets"]["plots"][0]).read_text(encoding="ascii")
    ...
    assert manifest["schema_version"] == "live-uk-bird-maps-historical-1.2"
    assert all(
        path.startswith(f"archive/historical/{manifest['release_id']}/")
        for path in [
            *manifest["assets"]["daily_by_year"].values(),
            manifest["assets"]["annual"],
            manifest["assets"]["phenology"],
            manifest["assets"]["coverage"],
            manifest["assets"]["boundary"],
            manifest["assets"]["release_manifest"],
            *manifest["assets"]["plots"],
        ]
    )
    assert not (output / "historical").exists()

    # A retry reuses the content-addressed immutable release instead of
    # rewriting objects that a public manifest may already reference.
    retry = build_historical_products(
        source,
        output,
        radars_path=radars,
        boundary_source=boundary,
    ...
    assert retry["release_id"] == result["release_id"]
    assert len(list((output / "archive/historical").iterdir())) == 1

    # A failed subsequent analysis cannot replace the last valid manifest.
    published = latest_path.read_text(encoding="utf-8")
    failed_analysis = json.loads((source / "analysis_summary.json").read_text(encoding="utf-8"))
    failed_analysis["failure_count"] = 1
    (source / "analysis_summary.json").write_text(json.dumps(failed_analysis), encoding="utf-8")
    with pytest.raises(ValueError, match="failure_count=0"):
        build_historical_products(
            source,
            output,
            radars_path=radars,
            boundary_source=boundary,
        )
    assert latest_path.read_text(encoding="utf-8") == published
...
def test_static_ui_is_historical_not_forecast() -> None:
    ...
    assert "function drawRadarMarkers(ctx, width, height)" in javascript
    ...
    assert "window.setTimeout(() => advanceAnimation(generation), 900)" in javascript
