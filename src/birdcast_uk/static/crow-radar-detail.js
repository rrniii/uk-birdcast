...
(function () {
  ...
  function profileMetrics(rows) {
    ...
    const sums = { vid: valid.reduce((total, row) => total + (Number.isFinite(row.dens) ? row.dens : 0), 0) * widthKm, vir: valid.reduce((total, row) => total + (Number.isFinite(row.eta) ? row.eta : 0), 0) * widthKm };
    if (rain.length >= 3 && rain.length / Math.max(low.length, 1) >= QC.rainFraction) return { mtr: NaN, rtr: NaN, ...sums };
    return { mtr: valid.reduce((total, row) => total + (Number.isFinite(row.ff) && Number.isFinite(row.dens) ? row.ff * row.dens * 3.6 : 0), 0) * widthKm, rtr: valid.reduce((total, row) => total + (Number.isFinite(row.ff) && Number.isFinite(row.eta) ? row.ff * row.eta * 3.6 : 0), 0) * widthKm, ...sums };
  }
  class CrowRadarDetail extends HTMLElement {
    ...
    constructor() { super(); this.attachShadow({ mode: "open" }); this.metric = "mtr"; this.data = null; this.request = 0; }
    ...
    render(message, error) {
      ...
      this.shadowRoot.innerHTML = `<style>:host{display:block;color:#e9efeb;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.box{border:1px solid #34403a;background:#0d1210}.head{display:flex;justify-content:space-between;align-items:start;flex-wrap:wrap;gap:9px;padding:12px 14px;border-bottom:1px solid #34403a}.title{font-weight:700;font-size:15px}.meta,.note{color:#aeb9b2;font-size:11px}.tools{display:flex;gap:6px}button,select{padding:6px 8px;background:#151c18;color:#edf4ef;border:1px solid #526159;font:inherit;font-size:11px}button.active{background:#e0a62e;color:#111;border-color:#e0a62e}.status{padding:11px 14px;color:${error ? "#ffb1a6" : "#bdc8c0"};font-size:12px}.chart{padding:10px 14px 14px}.chart[hidden]{display:none}.chart h4{margin:0 0 6px;font-size:12px}.canvas-wrap{overflow-x:auto}canvas{display:block;width:100%;min-width:540px;background:#080c0b;border:1px solid #263029}.note{padding:0 14px 13px}@media(max-width:650px){.tools{width:100%;justify-content:space-between}}</style><section class="box"><header class="head"><div><div class="title">${label} radar detail</div><div class="meta">CROW metrics · 200–4000 m · ${(this.getAttribute("pulse") || "lp").toUpperCase()} · 10-minute UTC</div></div><div class="tools"><button data-i="24" class="${interval === 24 ? "active" : ""}">1 day</button><button data-i="72" class="${interval === 72 ? "active" : ""}">3 days</button><select>${Object.entries(METRICS).map(([key, value]) => `<option value="${key}" ${key === this.metric ? "selected" : ""}>${key.toUpperCase()} · ${value[0]}</option>`).join("")}</select></div></header><div class="status">${message}</div><div class="chart" ${this.data ? "" : "hidden"}><h4>${METRICS[this.metric][0]} (${METRICS[this.metric][1]})</h4><div class="canvas-wrap"><canvas class="line" height="170"></canvas></div></div><div class="chart" ${this.data ? "" : "hidden"}><h4>Vertical profile density</h4><div class="canvas-wrap"><canvas class="heat" height="240"></canvas></div></div><p class="note">The raw VPTS archive stays in the Object Store. Rain-contaminated profiles are retained for density but excluded from MTR/RTR.</p></section>`;
      this.shadowRoot.querySelector("select").addEventListener("change", (event) => { this.metric = event.target.value; this.render(message, error); this.draw(); });
      this.shadowRoot.querySelectorAll("button[data-i]").forEach((button) => button.addEventListener("click", () => this.setAttribute("interval-hours", button.dataset.i)));
    }
    async load() {
      const radar = this.getAttribute("radar"); const template = this.getAttribute("object-url-template"); const selected = this.getAttribute("date"); if (!radar || !template || !selected) return;
      const id = ++this.request; this.data = null; this.render("Loading raw VPTS profiles from the Object Store…"); this.dispatchEvent(new CustomEvent("crow-loading", { bubbles: true }));
      const centre = new Date(`${selected}T00:00:00Z`); const first = this.interval() === 72 ? plusDays(centre, -1) : centre; const days = Array.from({ length: this.interval() / 24 }, (_, index) => plusDays(first, index));
      try { const text = await Promise.all(days.map(async (value) => { const response = await fetch(urlFor(template, radar, this.getAttribute("pulse") || "lp", value)); if (response.status === 404) return ""; if (!response.ok) throw new Error(`${response.status} for ${day(value)}`); return response.text(); })); if (id !== this.request) return; this.data = this.normalise(text.flatMap((value) => value ? rowsFromCsv(value) : [])); this.data.axisFirst = first.getTime(); this.data.axisLast = plusDays(first, days.length).getTime(); this.render(`${this.data.profiles.length} ten-minute profiles loaded. ${day(first)} to ${day(plusDays(first, days.length - 1))}.`); this.draw(); this.dispatchEvent(new CustomEvent("crow-ready", { detail: { radar, profiles: this.data.profiles.length, intervalHours: this.interval() }, bubbles: true })); } catch (error) { if (id === this.request) { this.render(`The selected VPTS files could not be loaded: ${error.message}`, true); this.dispatchEvent(new CustomEvent("crow-error", { detail: { radar, message: error.message }, bubbles: true })); } }
    }
